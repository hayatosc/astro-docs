...
コンテンツコレクションはZodのバリデーション、エディターのインテリジェンスと型チェックなどでTypeScriptに依存しています。Astroの `strict` または `strictest` TypeScript拡張を利用していない場合、 `tsconfig.json` 内の `compilerOptions` は次のように指定していることを確認する必要があります:
...
- `loader` for a data source (required)
