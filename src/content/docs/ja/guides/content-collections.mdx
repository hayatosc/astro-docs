...
**コンテンツコレクション**は、Astroプロジェクトで複数のコンテンツを管理するもっとも良い方法です。コレクションはドキュメントを整理し、エディターのインテリジェンスと型チェックを有効にし、コンテンツにTypeScriptの型安全を自動的に提供します。

Astro v5.0ではコンテンツコレクションを定義し、呼び出すためのコンテンツレイヤーAPIが登場しました。この機能的でスケーラブルなAPIは、ローカルに対してはビルトインのコンテンツローダーを提供します。リモートコンテンツに対しては、サードパーティやコミュニティ作成のローダーを利用したり、カスタムローダーを作成してあらゆるソースからデータを読み込んだりすることができます。

:::note
プロジェクトではAstro v2.0で登場した、レガシーのコンテンツコレクションAPIを使用し続けることができます。しかしながら、可能であれば[既存のコレクションのアップデート](/ja/guides/upgrade-to/v5/#legacy-v20-content-collections-api)をすることをおすすめします。
:::

## コンテンツコレクションとは？

構造的に似たデータの集合を **コレクション** として定義できます。これはブログポストのディレクトリや、複数の製品のJSONファイル、または同じ形状の複数のアイテムを表すデータです。

プロジェクト内またはファイルシステム上にローカルに保存されるコレクションには、Markdown、MDX、Markdoc、YAML、またはJSONファイルのエントリを含めることができます:

<FileTree>
- src/
- **newsletter/** "newsletter" コレクション
  - week-1.md コレクションエントリ
  - week-2.md コレクションエントリ
  - week-3.md コレクションエントリ
- **authors/** "author" コレクション
  - authors.json 全てのコレクションエントリを含めた単一のファイル
</FileTree>

適切なコレクションローダーを使用することで、CMS、データベース、ヘッドレス決済システムなどのあらゆる外部ソースのリモートデータにフェッチすることができます。

## コレクションのためのTypeScript構成

コンテンツコレクションはZodのバリデーション、エディターのインテリジェンスや型チェックなどでTypeScriptに依存しています。Astroの `strict` または `strictest` TypeScript拡張を利用していない場合、 `tsconfig.json` 内の `compilerOptions` は以下のように指定する必要があります:

```json title="tsconfig.json" ins={5} {6}
{
  // "astro/tsconfigs/strict" または "astro/tsconfigs/strictest" に含まれる
  "extends": "astro/tsconfigs/base",
  "compilerOptions": {
    "strictNullChecks": true, // `base` テンプレートを使用している場合
    "allowJs": true // 必須であり、すべてのAstroテンプレートに含まれている
  }
}
```
...
個々のコレクションは `defineCollection()` で指定します:
- `loader` データソースを読み込む (必須)
- `schema` 型安全にする (任意ですが、強く推奨されます) 

### コレクション設定ファイル

コレクションを定義するには、 `src/content.config.ts` (`.js`、`.mjs`拡張子もサポートしています) をプロジェクトに作成する必要があります。
...
// 4. `collections` オブジェクトにコレクションを登録し、エクスポート
export const collections = { blog, dogs };
```

### コレクションローダーの定義

コンテンツレイヤーAPIは、(プロジェクトのローカルかリモートかのどちらに保存されているかに関わらず)コンテンツにフェッチし、 `loader` プロパティを使用してデータを取得することができます。

#### ビルトインローダー

Astroは[2つのビルトインローダー関数](/ja/reference/content-loader-reference/#built-in-loaders) (`glob()`と`file()`)が用意されているほか、独自のローダーを構築してリモートデータを取得するためのAPIへのアクセスも用意されています。

[`glob()`ローダー](/ja/reference/content-loader-reference/#glob-loader) はファイルシステム上にあるMarkdown、MDX、Markdoc、JSON、YAMLファイルのディレクトリをエントリとして作成します。[micromatch](https://github.com/micromatch/micromatch#matching-features)でサポートされているglobパターンを用いてエントリファイルを探す `pattern` と、ファイルが配置されているベースファイルパスを指定します。各エントリの `id` はファイル名から自動的に生成されます。各エントリごとに1つのファイルがある場合、このローダーを使用してください。

[`file()` ローダー](/ja/reference/content-loader-reference/#file-loader) は1つのローカルファイルから複数のエントリを作成します。ファイル内のそれぞれのエントリにはユニークな `id` キープロパティを持っている必要があります。ファイルへの `base` ファイルパスと、オプションで、自動的に解析できないデータファイル用の [`parser` 関数](#parser-関数) が利用できます。パースした際にオブジェクトの配列となるようなファイルの場合はこのローダーを使用してください。

```ts  title="src/content.config.ts" {5,9}
import { defineCollection, z } from 'astro:content';
import { glob, file } from 'astro/loaders'; // レガシーAPIでは使用できません
...
const probes = defineCollection({
  // `loader` は文字列だけでなく複数のパターンを配列で受け取れます
  // space-probesディレクトリの中で、"voyager-"から始まるもの以外の全てのMarkdownファイルをロードします
...
`file()` ローダーは2つ目の引数に `parser` 関数を渡せます。これによりカスタムパーサー (e.g. `toml.parse` や `csv-parse`) を指定してファイルのコンテンツからコレクションを作成できます。

`file()` ローダーは自動的にJSONやYAMLファイルを(それらの拡張子によって)発見しパースするため、[ネストされたJSONドキュメント](#ネストされた-json-ドキュメント)でない限りは `parser` を使う必要はありません。`.toml`や`.csv`など、他のファイルを利用する場合は、パーサー関数を作成する必要があります。

次の例では `.toml` ファイルを使用してコンテンツコレクションの `dogs` を定義しています:
...
TOMLのパーサーをインポートしたら、 `file()` ローダーにファイルパスと `parser` 関数を渡すことで `dogs` コレクションをプロジェクトに読み込むことができます。同様のプロセスで、 `.csv` ファイルから `cats` コレクションを定義することができます:
...
`parser` 引数はネストされたJSONドキュメントから単一のコレクションを読み込むことができます。例えば、このJSONファイルには複数のコレクションが含まれています:

```json title="src/data/pets.json"
{"dogs": [{}], "cats": [{}]}
```

それぞれのコレクションの `file()` ローダーでカスタム `parser` を通すことで、これらのコレクションを分離することができます:
